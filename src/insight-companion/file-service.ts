...
- **Analysis Method:** ${metadata.chunksProcessed > 1 ? 'Multi-chunk with aggregation' : 'Single-pass analysis'}`;
	...
	/**
	 * Estimate actual cost based on tokens used
	 */
	private estimateActualCost(tokensUsed: { prompt: number; completion: number; total: number }): number {
		// GPT-4 pricing (update as needed)
