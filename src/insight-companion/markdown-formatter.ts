...
export interface FormattingConfig {
	...
	timeFormat: '12h' | '24h';
    // Optional plugin version to display in footer
...
export class MarkdownFormatter {
	...
	static formatSummary(summaryResult: SummaryResult, config: Partial<FormattingConfig> = {}): string {
		...
		sections.push('');
		sections.push('---');
		sections.push(`Generated by Insight Companion v${finalConfig.pluginVersion ?? 'dev'}`);
					...
					for (const item of value) {
						if (item && typeof item === 'object') {
							...
							if (entries.length === 0) {
								...
								// First line starts the object
								const [firstKey, firstVal] = entries[0];
								lines.push(`  - ${firstKey}: ${formatYamlValue(firstVal)}`);
								// Subsequent lines are indented under the dash
...
// Helpers to safely format YAML scalar values
function formatYamlValue(value: any): string {
    if (value === null || value === undefined) return 'null';
    if (typeof value === 'number' || typeof value === 'boolean') return String(value);
    const text = String(value);
    // Quote if value contains colon, leading special chars, or starts with {/[/- which YAML may parse oddly
    if (/[:#\-\[\]\{\},&*!?]|^\s|\s$/.test(text)) {
