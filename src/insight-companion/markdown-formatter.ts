...
export class MarkdownFormatter {
	...
	private static formatMetadataSection(metadata: SummaryResult['metadata'], config: FormattingConfig): string {
		const { dateRange, notesAnalyzed, tokensUsed, chunksProcessed, generationTime, model } = metadata;
		...
		sections.push(`- **Method:** ${chunksProcessed > 1 ? 'Multi-chunk with aggregation' : 'Single-pass analysis'}`);
		sections.push('');
