...
describe('NoteFilterService Tests', () => {
	...
	describe('metadata exclusion', () => {
		...
		it('should exclude notes with summarise: false in frontmatter by default', async () => {
			// Mock vault.read to return content with summarise: false in frontmatter
			app.vault.read = jest.fn().mockImplementation((file: MockTFile) => {
				if (file.path === 'note1.md') {
					return Promise.resolve('---\nsummarise: false\n---\nContent here');
				}
			...
			// Should exclude note1 due to summarise: false in frontmatter
			expect(result.totalCount).toBe(0);
			expect(result.notes).toHaveLength(0);
			expect(result.excludedMetadata).toEqual(['summarise: false']);
		});

		it('should include notes without summarise: false when using default exclusion', async () => {
			// Mock vault.read to return content without summarise: false
			app.vault.read = jest.fn().mockImplementation((file: MockTFile) => {
				if (file.path === 'note1.md') {
					return Promise.resolve('---\nstatus: active\n---\nContent here');
				}
			...
			// Should include note1 since it doesn't have summarise: false
			expect(result.totalCount).toBe(1);
			expect(result.notes).toHaveLength(1);
			expect(result.excludedMetadata).toEqual(['summarise: false']);
		});
