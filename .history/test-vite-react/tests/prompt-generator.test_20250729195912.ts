...
describe('PromptGenerator', () => {
	...
	describe('prompt structure and formatting', () => {
		...
		test('should include approach guidelines', () => {
			...
			expect(result.content).toContain('Only call out what you actually see');
			expect(result.content).toContain('keeps showing up');
			expect(result.content).toContain('flow naturally from what\'s written');
		});
