...
Some longer content to make the token estimation more realistic.
	...
	describe('Modal Integration in Workflow', () => {
		it('should open confirmation modal after note filtering and token estimation', async () => {
			...
			// Spy on ConfirmationModal constructor to verify it's called
			...
			// Verify that confirmation modal was created and opened
			expect(modalConstructorSpy).toHaveBeenCalled();
			expect(modalOpenSpy).toHaveBeenCalled();

			modalOpenSpy.mockRestore();
		});

		it('should pass correct data to confirmation modal', async () => {
			const dateRange: DateRange = {
				startDate: '2023-06-01',
				endDate: '2023-06-30'
			};

			// Create a mock constructor to capture the passed data
			...
			// Verify the confirmation data structure
			expect(capturedConfirmationData).toBeDefined();
			expect(capturedConfirmationData.filterResult).toBeDefined();
			expect(capturedConfirmationData.tokenEstimate).toBeDefined();
			expect(capturedConfirmationData.costEstimate).toBeDefined();
			expect(capturedConfirmationData.limitCheck).toBeDefined();

			// Verify filter result
			expect(capturedConfirmationData.filterResult.notes).toBeDefined();
			expect(capturedConfirmationData.filterResult.totalCount).toBeGreaterThan(0);
			expect(capturedConfirmationData.filterResult.dateRange).toEqual(dateRange);

			// Verify token estimate
			expect(capturedConfirmationData.tokenEstimate.totalTokens).toBeGreaterThan(0);
			expect(capturedConfirmationData.tokenEstimate.noteCount).toBe(capturedConfirmationData.filterResult.totalCount);

			modalConstructorSpy.mockRestore();
			modalOpenSpy.mockRestore();
		});

		it('should handle confirmation callback correctly', async () => {
			const dateRange: DateRange = {
				startDate: '2023-06-01',
				endDate: '2023-06-30'
			};

			// Mock the modal to capture callbacks
			...
			// Mock the modal creation to capture the callbacks
			const originalModal = ConfirmationModal;
			(global as any).ConfirmationModal = jest.fn().mockImplementation((app, confirmationData, onConfirm, onCancel) => {
			...
			// Spy on the proceedWithSummaryGeneration method
			const proceedSpy = jest.spyOn(plugin as any, 'proceedWithSummaryGeneration').mockImplementation();
			...
			// Verify proceedWithSummaryGeneration was called
			expect(proceedSpy).toHaveBeenCalled();

			// Restore
			(global as any).ConfirmationModal = originalModal;
			modalConstructorSpy.mockRestore();
			modalOpenSpy.mockRestore();
			proceedSpy.mockRestore();
		});

		it('should handle cancellation callback correctly', async () => {
			const dateRange: DateRange = {
				startDate: '2023-06-01',
				endDate: '2023-06-30'
			};
			...
			expect(consoleSpy).toHaveBeenCalledWith('User cancelled summary generation');
