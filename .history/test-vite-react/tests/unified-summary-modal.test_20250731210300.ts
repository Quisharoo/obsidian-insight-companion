...
describe('UnifiedSummaryModal', () => {
	...
	describe('onOpen', () => {
		...
		test('should create modal content with all sections', () => {
			const createElCalls = (modal.contentEl.createEl as jest.Mock).mock.calls;
			const createdElements = createElCalls.map(call => call[1]?.text || '');
			
			expect(createdElements).toContain('Summarise Notes');
			expect(createdElements).toContain('Date Range (Optional)');
			expect(createdElements).toContain('Folder (Optional)');
			expect(createdElements).toContain('Insight Style');
			expect(createdElements).toContain('Quick Date Presets');
		});
