...
export class FileService {
	...
	private generateFileName(summaryResult: SummaryResult): string {
		const { dateRange, folderName, mode, notesAnalyzed, chunksProcessed } = summaryResult.metadata;
		
		let baseName: string;
		
		// Create base filename based on mode
		if (mode === 'folder' && folderName) {
			// For folder mode, use folder name instead of date range
			baseName = this.config.fileNameTemplate.replace('{dateRange}', `Folder ${folderName}`);
		} else if (mode === 'date' && dateRange) {
			// For date mode, use date range
			...
			baseName = this.config.fileNameTemplate.replace('{dateRange}', dateRangeString);
		} else {
			// Fallback for edge cases
			baseName = this.config.fileNameTemplate.replace('{dateRange}', 'Summary');
		}
